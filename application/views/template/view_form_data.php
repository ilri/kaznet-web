...
<script>
	...
	function exportXcel(event) {
		...
		$.ajax({
			url: "<?php echo base_url(); ?>FormController/get_form_data_export/<?php echo $this->uri->segment(3); ?>",
			data: query_data,
			type: "POST",
			dataType: "JSON",
			...
			success: function(response) {
				...
				for (const key in fields) {
					...
					if(label != "Declaration" && type !="paragraph" && type !="button"){ //added by sagar to skip paragraph in display
						...
						tableHeaderFields.push(field);
					}
				}
				xcelHeader.push(...['Latitude','Longitude','Uploaded By','Uploaded Datetime','Status']); // Verification status
				tableHeaderFields.push(...['latitude','longitude','first_name','datetime','status']); // pa_verified_status
				...
				if(submitedData.length > 0){
					...
					for (let i=0; i<submitedData.length; i++){
						...
						elemnt.status = elemnt.status === '1' ? 'Submitted' : 'N/A'
						...
						for (let k = 0; k < tableHeaderFields.length; k++) {
							...
							if(lkpData[key]){
								...
								if(imageData[key]){
									...
									let rowValue = elemnt[key] || elemnt[key] || 'N/A';
									let selectedField = fields.filter(item=>item.name === key)
									if (selectedField && selectedField.length > 0 && selectedField[0].values?.length > 0) { //  && selectedField[0].type !== "autocomplete"
										let options = selectedField[0].values;
										// Parse if rowValue is a stringified array
										if (typeof rowValue === 'string') {
											try {
												...
												// Not a valid JSON, keep the original string
											}
										}
										const valueArray = Array.isArray(rowValue) ? rowValue : [rowValue];
										const labels = valueArray.map(value => {
											const option = options.find(opt => opt.value === value);
											return option ? option.label : value;
										});
										rowValue = Array.isArray(valueArray) && valueArray.length > 1 ? labels.join(', ') : labels[0];
									}
									row.push(rowValue);
								}
							}
					...
					exportToXcel(response.title??'Custom task', xcelData);
