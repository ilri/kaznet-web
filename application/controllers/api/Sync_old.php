...
class Sync extends CI_Controller {
	...
	public function download($data)
	{
		...
		if(!isset($data['user_id']) || !isset($data['fpo_fpc'])
		|| strlen($data['user_id']) == 0 || strlen($data['fpo_fpc']) == 0) {
			$this->jsonify(array(
				'status' => 0,
				'msg' => 'Bad Request! Could not verify user.'
			));
			exit();
		}
		...
		$tables = array(
			...
			'lkp_fpo' => 'fpo_id',
			'lkp_gender' => 'id',
			'lkp_variety' => 'VARIETY_ID',
			'lkp_ai_provider' => 'id',
			'lkp_ai_service_number' => 'id',
			'lkp_ai_types' => 'id',
			'lkp_animal_approximate_age' => 'id',
			'lkp_animal_body_condition' => 'id',
			'lkp_animal_breeds' => 'id',
			'lkp_animal_colors' => 'id',
			'lkp_animal_exit_types' => 'id',
			'lkp_animal_types' => 'id',
			'lkp_anmal_entry_type' => 'id',
			'lkp_birth_type' => 'id',
			'lkp_breeding_hormones' => 'id',
			'lkp_breeding_who_sync' => 'id',
			'lkp_breed_composition' => 'id',
			'lkp_calve_deformity' => 'id',
			'lkp_calve_status' => 'id',
			'lkp_calve_use' => 'id',
			'lkp_calving_method' => 'id',
			'lkp_calving_type' => 'id',
			'lkp_cow_type' => 'id',
			'lkp_ease_of_calving' => 'id',
			'lkp_farm_type' => 'id',
			'lkp_feed_method' => 'id',
			'lkp_health_disease' => 'id',
			'lkp_health_provider' => 'id',
			'lkp_health_status' => 'id',
			'lkp_hoof_health' => 'id',
			'lkp_hoof_problem' => 'id',
			'lkp_language' => 'id',
			'lkp_milk_sample_type' => 'id',
			'lkp_payment_mode' => 'id',
			'lkp_pd_method' => 'id',
			'lkp_pd_stage' => 'id',
			'lkp_person_age_group' => 'id',
			'lkp_person_relationship' => 'id',
			'lkp_pregnancy_diagnosis' => 'id',
			'lkp_project_origin' => 'id',
			'lkp_roster_education_level' => 'id',
			'lkp_semen_source' => 'id',
			'lkp_semen_supplier' => 'id',
			'lkp_semen_type' => 'id',
			'lkp_sire_owner' => 'id',
			'lkp_sire_type' => 'id',
			'lkp_synchronization_type' => 'id',
			'lkp_udder_score' => 'id',
			'lkp_vaccine_type' => 'id',
			'lkp_why_dead' => 'id',
			'query' => 'id',
			...
			'survey10' => 'id',
			'survey11' => 'id',
			'tbl_checkin_checkout' => 'inout_id',
			'tbl_fpo_fpc_account' => 'id',
			'tbl_fpo_fpc_location' => 'loc_id',
			'tbl_kmlfile' => 'kml_id',
			'tbl_kmlimage' => 'img_id',
			'lkp_ricircle' => 'ricircle_id'
		);
		...
		foreach($tables as $table => $pk) {
			...
			if($table != 'query') {
				...
				if($table == 'lkp_state') {
					$this->db->join('tbl_user_unit_location AS tuul', 'tuul.state_id = table.state_id');
					$this->db->where('tuul.status', 1)->where('tuul.user_id', $data['user_id']);
					$this->db->where('tuul.fpo_fpc_id', $data['fpo_fpc']);
				} else if($table == 'lkp_district') {
					$this->db->join('tbl_user_unit_location AS tuul', 'tuul.district_id = table.district_id');
					$this->db->where('tuul.status', 1)->where('tuul.user_id', $data['user_id']);
					$this->db->where('tuul.fpo_fpc_id', $data['fpo_fpc']);
				} /*else if($table == 'lkp_tehsil') {
					$this->db->join('tbl_user_unit_location AS tuul', 'tuul.tehsil_id = table.tehsil_id');
					$this->db->where('tuul.status', 1)->where('tuul.user_id', $data['user_id']);
					$this->db->where('tuul.fpo_fpc_id', $data['fpo_fpc']);
				}*/ else if($table == 'lkp_block') {
					$this->db->join('tbl_user_unit_location AS tuul', 'tuul.block_id = table.block_id');
					$this->db->where('tuul.status', 1)->where('tuul.user_id', $data['user_id']);
					$this->db->where('tuul.fpo_fpc_id', $data['fpo_fpc']);
				}/* else if($table == 'lkp_grampanchayat') {
					$this->db->join('tbl_user_unit_location AS tuul', 'tuul.grampanchayat_id = table.grampanchayat_id');
					$this->db->where('tuul.status', 1)->where('tuul.user_id', $data['user_id']);
					$this->db->where('tuul.fpo_fpc_id', $data['fpo_fpc']);
				} */else if($table == 'lkp_village') {
					$this->db->join('tbl_user_unit_location AS tuul', 'tuul.village_id = table.village_id');
					$this->db->where('tuul.status', 1)->where('tuul.user_id', $data['user_id']);
					$this->db->where('tuul.fpo_fpc_id', $data['fpo_fpc']);
				}

				// if($table == 'tbl_user_unit_location') {
				// 	$this->db->where('table.user_id', $data['user_id'])->where('table.status', 1);
				// }
				// switch ($table) {
				// 	case 'tbl_farmers':
				// 	case 'tbl_plot':
